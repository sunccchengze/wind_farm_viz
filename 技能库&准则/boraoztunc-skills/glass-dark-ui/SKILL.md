...
## Use When
- A dark interface needs frosted glass panels, blur shells, or gradient/glow borders.
- You need the *implementation* — tokens, `backdrop-filter` recipe, masked gradient
  border, contrast floors — for a dark glass surface.

**Choosing between the dark skills:** use this one for the concrete CSS of a glass
surface. Use `visual-style-presets` (`dark-glass-workspace` preset) when you need the
whole page direction — layout shell, structural motif, accent discipline. Use
`visual-style-presets` (`dark-technical-framed`) instead when the identity should come
from frames and rails rather than blur, and `mesh-gradient-dark-blue-clean` when the
background itself is an animated mesh field.
