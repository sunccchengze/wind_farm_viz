...
Both `mask` and `-webkit-mask` are required. `mask-composite: exclude` needs mask
layers declared on the **unprefixed** property to have anything to composite — ship
only `-webkit-mask` and spec-compliant engines get no mask at all, so the
`::before` renders as a solid rectangle covering the whole surface. The two
composite keywords are not interchangeable either: WebKit's legacy property takes
`xor`, the standard property takes `exclude`.
