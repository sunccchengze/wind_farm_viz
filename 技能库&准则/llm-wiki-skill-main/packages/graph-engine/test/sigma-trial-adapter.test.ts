...
import { buildSigmaGraphologyTrialModel } from "./sigma-trial-adapter";

describe("Sigma/Graphology trial adapter", () => {
  it("preserves graph semantics for the isolated WebGL candidate route", () => {
    ...
    const model = buildSigmaGraphologyTrialModel(data, {
      pins,
      searchResultIds: ["b"],
...
function graphFixture(): GraphData {
  return {
    meta: {
      ...
      wiki_title: "Sigma trial adapter",
      total_nodes: 3,
      total_edges: 2
    },
    nodes: [
